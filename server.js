...
// Beperkt het aantal inlogpogingen om wachtwoord-raden te bemoeilijken.
// Max 8 pogingen per 10 minuten per IP-adres.
const loginLimiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 8,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'Te veel inlogpogingen. Probeer het over een paar minuten opnieuw.' },
});
  ...
  const items = messages.length
    ? messages
      ...
      <pubDate>${new Date(m.createdAt).toUTCString()}</pubDate>
        ...
        .join('\n')
    : `    <item>
      <title>Op dit moment zijn er geen mededelingen</title>
      <description>Op dit moment zijn er geen mededelingen</description>
      <link>${escapeXml(SITE_URL)}</link>
      <guid isPermaLink="false">geen-mededelingen</guid>
